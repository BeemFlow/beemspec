...
| Story map get (full graph) | Yes (`storymap_get`) | Yes (`GET /api/story-maps/[id]`) | Both provide map + activities/tasks/stories/releases/personas. |
| Story map create | Yes (`storymap_create`) | Yes (`POST /api/story-maps`) | Near parity. |
| Story map update | Yes (`storymap_update`) | Yes (`PUT /api/story-maps/[id]`) | Near parity. |
...
| Release create/update/reorder | Yes (`release_create/update/reorder`) | Yes (`POST /api/releases`, `PUT /api/releases/[id]`, `PUT /api/releases`) | Parity. |
