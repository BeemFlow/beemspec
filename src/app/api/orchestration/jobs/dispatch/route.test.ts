...
vi.mock('@/orchestration/release-runner/jobs', () => ({ dispatchQueuedReleaseBuildJobs: vi.fn() }));
...
describe('orchestration dispatch route', () => {
  ...
  it('dispatches queued jobs', async () => {
    ...
    vi.mocked(dispatchQueuedReleaseBuildJobs).mockResolvedValue({ considered: 2, claimed: 2, completed: 2, failed: 0 });
  ...
  it('accepts worker token auth path', async () => {
    ...
    vi.mocked(createClient).mockResolvedValue({ from: vi.fn() } as never);
    vi.mocked(dispatchQueuedReleaseBuildJobs).mockResolvedValue({ considered: 1, claimed: 1, completed: 1, failed: 0 });
