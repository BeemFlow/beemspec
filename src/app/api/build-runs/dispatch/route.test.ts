...
vi.mock('@/build-runs/queue', () => ({ dispatchQueuedBuildRunJobs: vi.fn() }));

describe('build-run dispatch route', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it('dispatches queued jobs', async () => {
    ...
    vi.mocked(dispatchQueuedBuildRunJobs).mockResolvedValue({
      considered: 2,
      claimed: 2,
      completed: 2,
      requeued: 0,
    ...
    const response = await POST(new Request('http://localhost/api/build-runs/dispatch?limit=2', { method: 'POST' }));
  ...
  it('accepts worker token auth path', async () => {
    ...
    await POST(
      new Request('http://localhost/api/build-runs/dispatch', {
        method: 'POST',
