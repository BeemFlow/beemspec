...
export async function POST(request: Request) {
  ...
  if (latestItemError) return NextResponse.json({ error: 'Failed to locate release run item' }, { status: 500 });
  if (!latestItem) return NextResponse.json({ error: 'No release run item found for story' }, { status: 404 });
