...
/**
 * Build a generic LinearStoryPatch by parsing a Linear issue's
 * description (markdown sections) and mapping its state name.
 */
export function buildStoryPatchFromLinearIssue(input: LinearIssueForSync): LinearStoryPatch {
  const fromDescription = parseLinearDescriptionToStoryFields(input.description);
  const patch: LinearStoryPatch = {
